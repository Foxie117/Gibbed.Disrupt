...
namespace Gibbed.Disrupt.BinaryObjectInfo.FieldHandlers
{
    internal class Vector4Handler : ValueHandler<Vector4>
    {
        ...
        public override Vector4 Parse(string text)
        {
            ...
            if (Helpers.TryParseFloat32(parts[2], out z) == false)
            {
                throw new FormatException("failed to parse Float Z");
            }

            if (Helpers.TryParseFloat32(parts[3], out w) == false)
            {
        ...
        public override string Compose(Vector4 value)
        {
